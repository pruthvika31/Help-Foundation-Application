...
def main():
    ...
    imp=st.text_input('Enter the imports in terms of percentage of GDP')
    exp=st.text_input('Enter the exports in terms of percentage of GDP')
    inf=st.text_input('Enter the inflation rate in a country (%)')

    hel=st.text_input('Enter the health expenditure in terms of % of GDP')
    ch_m=st.text_input('Enter the no. of deaths per 100 births for <5 years')
    fer=st.text_input('Enter the avg children born to a each women in a country')
